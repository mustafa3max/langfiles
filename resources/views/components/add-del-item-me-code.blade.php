 <div>
     ...
     <button
         :title="Object.keys(meCode).includes('{{ $data->key }}') ? '{{ __('me_str.selected') }}' :
             '{{ __('me_str.select') }}'"
         x-show="isGroup"
         x-on:click="Object.keys(meCode).includes('{{ $data->key }}')?removeCode('{{ $data->key }}',  ['{{ Globals::languages()[0] }}','{{ Globals::languages()[1] }}']):addCode('{{ $data->key }}','{{ json_encode($keys) }}', ['{{ Globals::languages()[0] }}','{{ Globals::languages()[1] }}']);
